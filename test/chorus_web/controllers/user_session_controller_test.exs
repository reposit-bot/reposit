defmodule ChorusWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_chorus_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert Chorus.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
