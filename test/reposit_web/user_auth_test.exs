defmodule RepositWeb.UserAuthTest do
  ...
  @remember_me_cookie "_reposit_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RepositWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "require_sudo_mode/2" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      over_an_hour_ago = DateTime.utc_now(:second) |> DateTime.add(-61, :minute)
      user = %{user | authenticated_at: over_an_hour_ago}
      user_token = Accounts.generate_user_session_token(user)
