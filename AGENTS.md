...
- Out of the box, `core_components.ex` imports an `<.icon name="x" class="w-5 h-5"/>` component for Lucide icons. **Always** use the `<.icon>` component for icons, **never** use `Lucideicons` modules directly
...
- **Prefer DaisyUI components** (`btn`, `card`, `badge`, `input`, `alert`, `tabs`, etc.) as the foundation for UI elements. You can still be creative with layouts, animations, gradients, and custom styling on top of DaisyUI - but avoid recreating components that DaisyUI already provides well. See `DaisyUI.md` for available components.
